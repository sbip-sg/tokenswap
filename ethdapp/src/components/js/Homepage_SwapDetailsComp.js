...
export class Homepage_SwapDetailsComp extends Component {
    ...
    deploySmartContract = (e) => {
        ...
        const deployTx = myContract.deploy({
            data: code,
            arguments: [((this.props.location).state.detail).split("|")[4], ((this.props.location).state.detail).split("|")[8]]
        }).send({
            from: accounts[0]
        ...
        if (window.confirm('Are you sure you want to withdraw your currency used for this exchange and cancel this Smart Contract initiation requst?')) {
            axios({
                ...
                data: {
                    ...
                    Secret: ((this.props.location).state.detail).split("|")[8]
                },
