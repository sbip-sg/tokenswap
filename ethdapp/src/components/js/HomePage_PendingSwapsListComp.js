...
export const HomePage_PendingSwapsListComp = () => {
                                ...
                                <div className="shadow max-h-96 overflow-auto border-b border-gray-200 sm:rounded-lg">
                                    <table className="min-w-full">
                                        ...
                                        <tbody className="divide-y divide-gray-300">
                                            {pendingSwapList.map((pendingSwap) => (
                                                <tr key={pendingSwap.addressPartyA} className="cursor-pointer" onClick={() => {
                                                    history.push({
                                                        pathname: "/dashboard/swap-details",
                                                        state: { detail: pendingSwap.namePartyA+"|"+pendingSwap.addressPartyA+"|"+pendingSwap.namePartyB+"|"+pendingSwap.addressPartyB+"|"+pendingSwap.swapRate+"|"+pendingSwap.swapStatus+"|"+pendingSwap.lastTransactionDate }
                                                    });
                                                }}>
                                                    <td className="px-4 py-4 bg-white text-gray-900 text-sm whitespace-nowrap">{pendingSwap.namePartyA}<br/>({pendingSwap.addressPartyA})</td>
                                                    <td className="px-4 py-4 bg-white text-gray-900 text-sm whitespace-nowrap">{pendingSwap.namePartyB}<br/>({pendingSwap.addressPartyB})</td>
                                                    <td className="px-5 py-4 bg-white text-gray-900 text-sm whitespace-nowrap">{pendingSwap.swapRate}</td>
                                                    <td className="px-5 py-4 bg-white text-gray-900 text-sm whitespace-nowrap">{pendingSwap.swapStatus}</td>
                                                    <td className="px-4 py-4 bg-white text-gray-900 text-sm whitespace-nowrap">{pendingSwap.lastTransactionDate}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
